@extends('layouts.app')
@section('title', env('APP_NAME').' - '.trans('messages.show').' '.trans('messages.propertyStatus'))

@section('content')
    <!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-fw fa-home"></i> @lang('messages.show') @lang('messages.propertyStatus') - {{ $propertyStatus->name }}
        </h1>
    </div>
    <!-- End Page Heading -->
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" id="name" class="form-control" value="{{ $propertyStatus->name }}" placeholder="@lang('messages.name')..." readonly>
                    </div>
                    <div class="form-group">
                        <label for="status">@lang('messages.name')</label>
                        <input type="text" id="status" class="form-control" value="@if($propertyStatus->status) @lang('messages.enabled') @else @lang('messages.disabled') @endif" readonly>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="created_at">@lang('messages.created_at')</label>
                        <input type="text" id="created_at" class="form-control" value="{{ $propertyStatus->created_at }}" placeholder="@lang('messages.created_at')..." readonly>
                    </div>
                    <div class="form-group">
                        <label for="created_by">@lang('messages.created_by')</label>
                        <input type="text" id="created_by" class="form-control" value="{{ $propertyStatus->createdBy->full_name }}" placeholder="@lang('messages.created_by')..." readonly>
                    </div>
                </div>
            </div>
        </div>
